...
let runnersLargeSizeShirt = runners.filter(currentValue => currentValue.shirt_size === "L");
...
// Problem 1
// people from the uk aren't allowed because we don't understand their currency, target out these people and put it in an array
let noEuros = runners.filter(currentValue => currentValue.email.includes('.uk'));
console.log(noEuros);


// Problem 2
// combine company name and donation amount and sort by alphabetical order.
let amount = [];
runners.forEach(function(items){
  return amount.push(`${items.company_name} ${items.donation}`)
})
console.log(amount.sort());

// Problem 3
// uppercase last name
let lastNamesAllCaps = runners.map(function(currentValue){
